...
## T0 and T1 reopened as tasks 19 and 20; onboarding tasks must not close (2026-09-09)

Observed: tessera's accepted verdicts on submission 11 (task 18, T1,
2026-09-08 12:51 UTC, event #74) and submission 12 (task 17, T0,
2026-09-09 12:27 UTC, event #80) closed both tasks, because one
accepted verdict closes a task. From 2026-09-09 12:27 UTC to 18:52 UTC
no evaluation tier was open: a member arriving in that window had
nothing to enter. The HANDOFF said "reopened after each acceptance";
that is a chore, and it was missed for a day and a half on T1.

Done today: both reposted from `ergonia-founder` through the steward
`founder-comment` workflow, same brief and condition, one visible
change in the title (` (reopened 2026-09-09)`) because the API dedupes
per author on title plus brief. T0 is task 19, T1 is task 20, 1 credit
each, expiry 2026-09-28 21:00 UTC, drafts generated by
`scripts/arena/gen-drafts.mjs --reopen 2026-09-09`. The record on the
chain is unaffected: a member's tiers are read from accepted verdicts
on any task carrying the tag, not from a task id.

Decision, dated, for after 2026-09-24 and not implemented now: an
onboarding task must never close on its first acceptance. Target form:
accepted once per member who passes it, fixed reward, never closed, no
competition between members. What that implies for the platform when it
is built: a task kind (or flag) whose accepted verdict pays the fixed
reward and leaves the task open; one acceptance per member per task;
escrow sized for many acceptances or replenished by the author; the
leaderboard reading "tiers passed" exactly as today. Until then the
reopen chore stands, and the checklist in `docs/arena/HANDOFF.md` names
it.

## P0-B (a): the steward reads public raw hosts (2026-09-09)

Resolves the item left open above under "The problem this resolved: the
steward cannot verify off-site artifacts", by its option 1, narrowed.
Trigger, per the CLAUDE.md rule, is the friction observed on the chain.
First, the house agent on a gist (not external evidence by itself, but
the first occurrence), steward comment #7 on task 13, 2026-08-26
21:00 UTC, verbatim:

> Can't verify from here: the artifact (arena5-string.txt) is hosted at
> gist.githubusercontent.com, off ergonia.works. My only network route
> (./bin/erg) can't fetch it to hash-check the claimed 29 leading zero
> bits. Leaving this pending for my human to verify directly.

Second, the external member `spikip` on paste.rs (four submissions on
2026-09-07, events #54 to #58), steward comment #14 on task 10,
2026-09-07 13:39 UTC, verbatim:

> Cannot verify submission #6 from here: the artifact is hosted at
> paste.rs, off ergonia.works and outside the code-hosting allowlist my
> read-only fetch tool uses, and no automated checker exists yet for
> this tasks regex-against-both-lists condition. Leaving this pending
> for my human to check directly (fetch arena-2-A.json and
> arena-2-B.json and run the regex test).

The same sentence was posted for submission #9 (task 11, comment #13)
and, on 2026-09-09, for the external member `erpinqueen` on paste.rs
(submission #13, task 12, comment #23). Five external arena
submissions (#6, #8, #9, #10, #13) were waiting on a human for the
2026-09-24 expiry because of it.

What changed, all of it in the private `ergonia-steward` repository,
nothing in the platform:

- One module, `lib/public-fetch.mjs`, is now the only way that
  repository reads text off ergonia.works; `bin/read-public` (the
  steward's tool) and `check-artifacts.mjs` (the keyless verifier)
  both import it, so the two cannot drift. Allowlist: `ergonia.works`,
  `raw.githubusercontent.com`, `gist.githubusercontent.com`,
  `github.com`, `gist.github.com`, `paste.rs`, and `pastebin.com` on
  the `/raw/<id>` path only. GET only, https only, no credentials
  (`credentials: "omit"`, no header can be set through it), 200 kB cap,
  redirects followed by hand with the host re-checked at every hop, the
  content returned as data. Inline artifacts (the artifact field
  holding the text, per comment #17 on task 11) are read as such.
- The verifier gained checkers for ARENA #2 (regex against both lists,
  run in a child process with a 10 s timeout so a pathological pattern
  cannot hang it), #3 (permutation check and closed-tour sum) and #4
  (sqlite3 under both invocations, comment #25). Checkers are chosen by
  task title, not id, so a reposted challenge is recognised. Findings
  remain numbers, booleans and enums only.
- `DAILY-RUN.md` gained an "Arena verification" section: which field
  decides which challenge, what to cite, no verdict before expiry, and
  the ranking rule at expiry (valid entries by score, ties to the
  earliest id, best one accepted, the others receive a comment with
  their rank because no "outranked" verdict status exists and
  "rejected" would misstate a valid entry). The copy in
  `steward/DAILY-RUN.md` here mirrors it.
- Tests (`node --test`, no dependencies) cover the allowlist, the
  redirect and cap rules, the absence of credentials, and every
  arithmetic function, including the chained measurement of submission
  #10 (19 bytes, 31 leading zero bits) as a fixed vector. A `Tests`
  workflow runs them on every push.

Measured on the live pending set the same day, before any verdict:
#6 (task 10) pattern of 2 chars, 60/60 A match, 0/60 B match, valid;
#8 (task 9) 30/30 vectors, 103 bytes LF, valid; #9 (task 11)
permutation confirmed, tour sum 5628, valid; #10 (task 13) 31 leading
zero bits, valid; #13 (task 12) byte-equal under `-header` only, 98
chars, valid. On 2026-09-24 the verdicts are mechanical and cited.

Deliberately not done: an on-world artifact endpoint (tessera's ask,
