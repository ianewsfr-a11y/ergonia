...
// Each written file is also captured for the Worker embed module
// (src/arena-embed.ts) so `GET /arena-data/<file>` can serve the exact
// same bytes from the Worker.
const embed = /** @type {Array<{name:string,body:string,contentType:string}>} */ ([]);

function contentTypeFor(name) {
  if (name.endsWith(".json")) return "application/json; charset=utf-8";
  if (name.endsWith(".js") || name.endsWith(".mjs")) return "application/javascript; charset=utf-8";
  if (name.endsWith(".sql")) return "application/sql; charset=utf-8";
...
function write(name, content) {
  ...
  // README.md documents arena-data/ for humans on GitHub. It is not
  // served by the worker — the worker embed is challenge assets only.
  if (name !== "README.md") embed.push({ name, body: content, contentType: contentTypeFor(name) });
}
...
// -----------------------------------------------------------------
// src/arena-embed.ts — the Worker-side view of these assets.
// -----------------------------------------------------------------
{
  const src = resolve(ROOT, "src", "arena-embed.ts");
  let out = `// AUTOGENERATED by scripts/gen-arena-data.mjs — do not edit.
//
...
`;
  for (const f of embed) {
    // Backtick-escape any literal backticks in the body.
    const escaped = f.body.replace(/\\/g, "\\\\").replace(/`/g, "\\`").replace(/\$\{/g, "\\${");
    out += `  ["${f.name}", { name: ${JSON.stringify(f.name)}, contentType: ${JSON.stringify(f.contentType)}, body: \`${escaped}\` }],\n`;
  }
  out += `]);\n`;
  writeFileSync(src, out, { encoding: "utf8" });
  console.log("  wrote src/arena-embed.ts");
}
